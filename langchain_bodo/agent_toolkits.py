"""Agent for working with Bodo DataFrames objects.
Similar to https://github.com/langchain-ai/langchain-experimental/blob/main/libs/experimental/langchain_experimental/agents/agent_toolkits/pandas/base.py
"""
...
from langchain_bodo.prompt import (
    FUNCTIONS_WITH_DF,
...
def create_bodo_dataframes_agent(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        df: Pandas or Bodo Dataframe or list of Dataframes.
    ...
    Example:
        .. code-block:: python
            ...
            from langchain_bodo.agents import create_bodo_dataframes_agent
            import bodo.pandas as pd

            df = pd.read_csv("titanic.csv")
            llm = ChatOpenAI(model="gpt-3.5-turbo", temperature=0)
            agent_executor = create_bodo_dataframes_agent(
                llm,
                df,
    ...
    for _df in df if isinstance(df, list) else [df]:
        if not isinstance(_df, pd.DataFrame):
            raise ValueError(f"Expected a DataFrame, got {type(_df)}")

    # Convert Pandas Dataframes to Bodo DataFrames
    if isinstance(df, list):
        df = [
